...
// on-disk location: rmp = IP0/RMP at the key root, mm = IP1/MM under mm/. Both
// bundles build an Encryptor on open: the RMP one encrypts flat-tier rmp_item
// payloads, the MM one compact-encrypts clustered-tier mm_item payloads via
// EncryptRow. The clustered search/decrypt path (Search response over MM) is not
// wired yet — the MM bundle carries no Decryptor today.
type keyKind struct {
	name     string
	preset   string
	evalMode string
...
var (
	rmpKind = keyKind{name: "rmp", preset: "ip0", evalMode: "rmp", subdir: ""}
...
// KeysExist reports whether the requested key parts (default: all three) are
// present under WithKeyPath, in either the .json or legacy .bin form.
...
// GenerateKeys writes a fresh key set at WithKeyPath: the RMP (IP0) trio as JSON
// envelopes at the root (EncKey.json, EvalKey.json, SecKey.json) and the MM (IP1)
// trio as raw .bin under mm/. Both are generated by default — RegisterKeys needs
// both eval keys. Returns ErrKeysAlreadyExist if either set is already present;
// it never overwrites.
func GenerateKeys(opts ...KeysOption) error {
	...
	if anySlotExists(o.Path) || anySlotExists(filepath.Join(o.Path, mmKind.subdir)) {
	...
	defer func() {
		if !success {
			removeKeyArtifacts(o.Path)
			mmDir := filepath.Join(o.Path, mmKind.subdir)
			removeKeyArtifacts(mmDir)
			_ = os.Remove(mmDir) // best-effort; only removes it if now empty
		}
	}()

	// Flat tier: JSON envelopes at the root (the historical layout).
	// The eval mode (RMP | FLAT) follow WithFlatMode
	if err := generateBundle(o, flatBundleKind(o.FlatMode), true); err != nil {
		return err
	}
	// MM: raw .bin under mm/ — the eval key is hundreds of MB, so skip the
	// base64 JSON envelope that would inflate it further.
	if err := generateBundle(o, mmKind, false); err != nil {
		return err
	}
	success = true
	return nil
...
// (root) and MM (mm/) bundles. Both JSON envelopes and raw .bin files are
